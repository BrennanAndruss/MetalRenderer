...
vertex VertexOut vertexFunction(Vertex in [[stage_in]],
                                constant float4x4& viewProjMatrix   [[buffer(0)]],
                                constant ModelMatrix& model         [[buffer(1)]]) {
    VertexOut out;
    ...
    out.T = normalize(model.normalMatrix * in.tangent.xyz);
    out.N = normalize(model.normalMatrix * in.normal);
    out.B = cross(out.N, out.T) * in.tangent.w;
...
inline float3 phongLighting(float3 worldPosition,
                            float3 diffuseColor,
    ...
    // Diffuse
    float3 dC = max(dot(normal, -lightDirection), 0.0);
    float3 diffuse = lightDiffuse * dC * diffuseColor;
    
    // Specular
    float3 viewDir = normalize(viewPosition - worldPosition);
    float3 reflectDir = reflect(lightDirection, normal);
    float3 sC = pow(max(dot(viewDir, reflectDir), 0.0), 32);
    float3 specular = lightSpecular * sC * specularColor;
...
fragment float4 fragmentFunction(VertexOut in [[stage_in]],
                                 ...
                                 constant MaterialInfo& materialInfo    [[buffer(2)]],
    ...
    // Alpha clipping
    float4 diffuseColor = (materialInfo.hasDiffuseTexture
                           ? diffuseTexture.sample(colorSampler, in.texCoord) : float4(1.0));
    if (diffuseColor.a < 0.5) discard_fragment();
    
    float specularColor = (materialInfo.hasSpecularTexture
                           ? specularTexture.sample(colorSampler, in.texCoord).r : 1.0);
    
    // Normal mapping
    float3 tangentSpaceNormal = (materialInfo.hasNormalTexture
                                 ? normalTexture.sample(colorSampler, in.texCoord).rgb : float3(0.5, 0.5, 1.0));
    ...
    // Shadow mapping
    float4 lightSpacePosition = lightViewProjMatrix * float4(in.worldPosition, 1.0);
    lightSpacePosition.xyz /= lightSpacePosition.w;
    float2 lightSpaceCoord = lightSpacePosition.xy * 0.5 + 0.5;
    ...
    for (int i = 0; i < SHADOW_SAMPLE_COUNT; i++) {
        ...
        visibility += shadowMap.sample_compare(shadowSampler, coord, lightSpacePosition.z - SHADOW_BIAS);
    }
...
vertex float4 shadowVertexFunction(Vertex in [[stage_in]],
                                   constant float4x4& viewProjMatrix [[buffer(0)]],
                                   constant float4x4& modelMatrix [[buffer(1)]])
{
    return viewProjMatrix * modelMatrix * float4(in.position, 1.0);
}
